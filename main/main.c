...
#ifdef CONFIG_CSI_FILTER_AP_BSSID
#define CSI_FILTER_AP_BSSID_ENABLED 1
#else
#define CSI_FILTER_AP_BSSID_ENABLED 0
#endif

#ifdef CONFIG_CSI_FILTER_STA_DST_MAC
#define CSI_FILTER_STA_DST_MAC_ENABLED 1
#else
#define CSI_FILTER_STA_DST_MAC_ENABLED 0
#endif

#ifdef CONFIG_CSI_FILTER_HT_FRAMES
#define CSI_FILTER_HT_FRAMES_ENABLED 1
#else
#define CSI_FILTER_HT_FRAMES_ENABLED 0
#endif
...
#if CSI_FILTER_AP_BSSID_ENABLED
...
#if CSI_FILTER_STA_DST_MAC_ENABLED
...
#if CSI_FILTER_HT_FRAMES_ENABLED
  if (rx->sig_mode != 1) {
...
static esp_err_t wifi_init_sta(void) {
  ...
  ESP_RETURN_ON_ERROR(esp_wifi_get_mac(WIFI_IF_STA, sta_mac), TAG,
                      "get STA MAC failed");
  sta_mac_valid = true;
  ESP_LOGI(TAG, "Connected AP BSSID: " MAC_STR ", channel: %u",
           MAC_ARG(connected_ap_bssid), ap_info.primary);
  ESP_LOGI(TAG, "Station MAC: " MAC_STR, MAC_ARG(sta_mac));
  return ESP_OK;
}
...
static void csi_rx_callback(void *ctx, wifi_csi_info_t *info) {
  ...
  printf("CSI_DATA,%d,%" PRIu32 ",%" PRId64 ",%" PRIu32 "," MAC_STR ","
         ...
         MAC_ARG(info->mac),
         MAC_ARG(info->dmac), info->first_word_invalid ? 1 : 0, info->rx_seq,
         info->payload_len, rx->rssi, rx->channel, rx->secondary_channel,
         rx->rate, rx->sig_mode, rx->mcs, rx->cwb, rx->stbc, rx->sgi,
         rx->noise_floor, rx->ant, rx->sig_len, rx->rx_state, info->len);
...
static esp_err_t csi_init(void) {
  ...
  ESP_LOGI(TAG,
           "CSI enabled, mode: %s, filters: ap_bssid=%s, sta_dst=%s, ht=%s",
           csi_mode_name(), CSI_FILTER_AP_BSSID_ENABLED ? "on" : "off",
           CSI_FILTER_STA_DST_MAC_ENABLED ? "on" : "off",
           CSI_FILTER_HT_FRAMES_ENABLED ? "on" : "off");
  ESP_LOGI(TAG,
           "CSI_DATA,node_id,seq,local_time_us,rx_timestamp_us,src_mac,dst_mac,"
...
static esp_err_t start_gateway_ping(void) {
  ...
  ESP_RETURN_ON_ERROR(esp_ping_new_session(&ping_config, &callbacks,
                                           &gateway_ping),
                      TAG, "create gateway ping session failed");
  ESP_RETURN_ON_ERROR(esp_ping_start(gateway_ping), TAG,
                      "start gateway ping failed");
  ESP_LOGI(TAG,
           "Internal gateway ping started: target=" IPSTR
...
#else
  ESP_LOGI(TAG, "Internal gateway ping disabled");
#endif
  return ESP_OK;
}
...
static void csi_stats_task(void *arg) {
  ...
  while (true) {
    ...
    ESP_LOGI(TAG,
             ...
             csi_filtered_sig_mode_count, csi_null_count,
             csi_rx_seq_gap_count, ping_fps, timeouts - last_timeouts,
             timeouts);
